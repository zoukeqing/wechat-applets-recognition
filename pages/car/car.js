      ...
      success(res) {
        if (!res.authSetting['scope.writePhotosAlbum']) {
          ...
          while (num >= 0) {
            ...
            ctx.fillText(wordsResult[num].year, 85 * sc, h * 0.95 - 15 * i * sc)
            ctx.fillText(wordsResult[num].score + '%', 145 * sc, h * 0.95 - 15 * i * sc)
            num--;
            i++;
          }
          ctx.fillText('名称：', 10 * sc, h * 0.95 - 15 * i * sc);
          ctx.fillText('年份：', 85 * sc, h * 0.95 - 15 * i * sc)
          ctx.fillText('置信度：', 145 * sc, h * 0.95 - 15 * i * sc)
          ctx.draw()
        }
      }
    })
