Page({
  ...
  onLoad: function (options) {
    ...
    for (var i = 0; i < wordsResult.length; i++) {
      wordsResult[i].probability = (wordsResult[i].probability * 100).toFixed(2);
    }
    this.setData({
      ...
      success(res) {
        if (!res.authSetting['scope.writePhotosAlbum']) {
          ...
          while (num >= 0) {
            ...
            ctx.fillText(wordsResult[num].probability + '%', 70 * sc, h * 0.95 - 15 * i * sc);
            ctx.fillText(wordsResult[num].calorie + 'KJ/100g', 120 * sc, h * 0.95 - 15 * i * sc);
            num--;
            i++;
          }
          ctx.fillText('名称：', 10 * sc, h * 0.95 - 15 * i * sc);
          ctx.fillText('置信度：', 70 * sc, h * 0.95 - 15 * i * sc)
          ctx.fillText('热量：', 120 * sc, h * 0.95 - 15 * i * sc)
          ctx.draw()
        }
      }
    })
